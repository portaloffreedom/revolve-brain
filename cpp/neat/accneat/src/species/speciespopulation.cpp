...
size_t
SpeciesPopulation::size()
{
  return norgs;
}

Organism *
SpeciesPopulation::get(size_t index)
{
  return &orgs.curr()[index];
}

unique_ptr<Organism>
SpeciesPopulation::make_copy(size_t index)
{
...
#pragma omp parallel for
    ...
    for (SpeciesOrganism &org: orgs.curr()) {
      if (not org.species) {
        ...
        if (not org.species) {
          org.species = new Species(++last_species,
                                    true);
