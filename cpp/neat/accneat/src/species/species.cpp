...
{
  erase_if(organisms, [](SpeciesOrganism *org)
  {
...
{
  erase_if(organisms, [gen](SpeciesOrganism *org)
  {
                        ...
                        vector<Species *> &sorted_species)
{
  ...
  // If we have a super_champ (SpeciesPopulation champion),
  // finish off some special clones
  if (ioffspring < thechamp->super_champ_offspring)
  {
    ...
    if (ioffspring < (thechamp->super_champ_offspring - 1))
    {
      ...
      {
        genome_manager->mutate(new_genome, GenomeManager::MUTATE_OP_STRUCTURE);
      }
    }
  }
