...
rng_t::test()
{
  ...
  // posneg
  {
    ...
    assert_equals(real_t(count[0]) / N,
                  0.5);
  }
  ...
  // gauss
  {
    ...
    for (size_t i = 0; i < N; i++) {
      ...
      size_t bin = min(size_t(abs(x)),
                       NBINS - 1);
      count[bin]++;
    }

    assert_equals(real_t(count_neg[0]) / N,
                  0.3413);
    assert_equals(real_t(count_neg[1]) / N,
                  0.4772 - 0.3413);
    assert_equals(real_t(count_neg[2]) / N,
                  0.5 - 0.4772);
    assert_equals(real_t(count_pos[0]) / N,
                  0.3413);
    assert_equals(real_t(count_pos[1]) / N,
                  0.4772 - 0.3413);
    assert_equals(real_t(count_pos[2]) / N,
                  0.5 - 0.4772);
  }
