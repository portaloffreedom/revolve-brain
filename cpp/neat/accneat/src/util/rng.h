...
public:
    ...
    template <typename T>
    size_t
    index(std::vector<T> &v,
          size_t begin = 0)
    {
      std::uniform_int_distribution<int> dist(begin,
                                              v.size() - 1);
      return dist(engine);
    }
    ...
    prob()
    {
      std::uniform_real_distribution<real_t> dist(0,
                                                  1);
      return dist(engine);
    }
    ...
    class prob_switch_t
    {
        ...
        prob_switch_t(real_t x_) :
                x(x_)
        {}

    public:
        bool
        prob_case(real_t prob)
        {
          if (x < prob) {
            return true;
          } else {
