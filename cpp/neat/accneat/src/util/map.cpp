...
bool
LocationTranslator::try_find(std::string row,
                             std::string col,
                             Location &result)
{
  if (::try_find(row_index,
                 row,
                 result.index.row)) {
    if (::try_find(col_index,
                   col,
                   result.index.col)) {
...
parse_equals(Line &line)
{
  size_t pos = line.text.find_first_of('=');
  if (pos == string::npos) {
    line.err("Expecting '='");
  }
  string lhs = trim(line.text.substr(0,
                                     pos));
...
static void
parse_glyphs_attrs(Section &section,
...
static map<char, Glyph>
parse_glyphs(Section &section_glyphs,
             Section &section_attrs)
{
  ...
  parse_glyphs_attrs(section_attrs,
                     glyphs);
...
static void
parse_objects_attrs(Section &section,
                    Map &map)
{
  for (Line &line: section.lines) {
    pair<string, string> def = parse_equals(line);
              ...
              map<char, Glyph> &glyphs)
{
...

  {
    ...
    for (char c: labels) {
      string label = str(c);
      if (contains(map.loc_trans.col_index,
                   label)) {
  ...
  parse_objects_attrs(section_attrs,
                      map);
