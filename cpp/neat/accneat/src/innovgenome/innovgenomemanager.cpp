...
bool
InnovGenomeManager::are_compatible(Genome &genome1,
                                   Genome &genome2)
{
...
void
InnovGenomeManager::clone(Genome &orig,
                          Genome &clone)
{
...
void
InnovGenomeManager::mate(Genome &genome1,
                         Genome &genome2,
                         Genome &offspring,
                         real_t fitness1,
                         real_t fitness2)
...
void
InnovGenomeManager::mutate(Genome &genome_,
...
void
InnovGenomeManager::finalize_generation(bool new_fittest)
{
  innovations.apply();
...
InnovGenomeManager::create_innov_func(Genome &g)
{
  return [this, &g](InnovationId id,
                    InnovationParms parms,
                    IndividualInnovation::ApplyFunc apply)
  {
      innovations.add(IndividualInnovation(g.genome_id,
                                           id,
                                           parms,
                                           apply));
  };
...
bool
InnovGenomeManager::is_mate_allowed()
{
...
bool
InnovGenomeManager::is_add_allowed()
{
  switch (env->search_type) {
    case GeneticSearchType::PHASED:
      return search_phase == COMPLEXIFY;
...
bool
InnovGenomeManager::is_delete_allowed()
{
  switch (env->search_type) {
    case GeneticSearchType::PHASED:
