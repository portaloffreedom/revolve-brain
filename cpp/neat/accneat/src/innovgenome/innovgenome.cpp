        ...
        : InnovGenome(robot_name)
{
...

  {
    ...
    // Bias node
    add_node(nodes,
             InnovNodeGene(NT_BIAS, node_id++, robot_name, robot_name, -1));

    // Sensor nodes
    for (size_t i = 0; i < ninputs; i++)
    {
      add_node(nodes,
               InnovNodeGene(NT_SENSOR, node_id++, robot_name, robot_name, -1));
    }
    ...
    {
      add_node(nodes,
               InnovNodeGene(NT_OUTPUT, node_id++, robot_name, robot_name, -1));
    }
    ...
    {
      add_node(nodes,
               InnovNodeGene(NT_HIDDEN, node_id++, robot_name, robot_name, -1));
    }
  }
  ...
  {
    add_link(links,
             InnovLinkGene(rng.element(traits).trait_id,
                           rng.prob(),
                           node_id_bias,
                           i + node_id_hidden,
                           false,
...
void InnovGenome::verify()
{
  ...
  for (InnovLinkGene &gene: links)
  {
    ...
    {
      if (&gene not_eq &gene2)
      {
        assert((gene.is_recurrent() not_eq gene2.is_recurrent())
               or (gene2.in_node_id() not_eq gene.in_node_id())
               or (gene2.out_node_id() not_eq gene.out_node_id()));
      }
    }
  }
#endif
...
bool NEAT::InnovGenome::load(std::istream &in)
{
  ...
  YAML::Node header = config["header"];
  if (header["type"].as< std::string >() not_eq YAML_HEADER_TYPE
      or header["version"].as< int >() not_eq YAML_HEADER_VERSION)
  {
...
int InnovGenome::LastNodeId()
{
  return nodes.back().node_id + 1;
}

real_t InnovGenome::LastGeneInnovationNumber()
{
...
void InnovGenome::mutate_link_weights(
        real_t power,
        real_t rate,
        mutator mut_type)
{
  ...
  for (InnovLinkGene &gene: links)
  {
    ...
    if (not(gene.frozen))
    {
      ...
      else if ((gene_total >= 10.0) and (num > endpart))
      {
    ...
    else
    {
      ...
      for (InnovLinkGene &checkgene: links)
      {
        if ((checkgene.in_node_id() == gene.in_node_id())
            and checkgene.enable
            and (checkgene.innovation_num not_eq gene.innovation_num))
        {
          found = true;
          break;
        }
        ...
        bool delete_split_link)
{
  ...
  auto innov_apply =
          [this, delete_split_link, splitlink](const Innovation *innov)
          {
            InnovNodeGene newnode(NT_HIDDEN, innov->newnode_id, *robot_name);
          ...
          std::remove_if(links.begin(),
                         ...
                         [&node](const InnovLinkGene &link)
                         {
                           return link.in_node_id() == node.node_id
                                  or link.out_node_id() == node.node_id;
                         });
...
bool InnovGenome::mutate_add_link(
        CreateInnovationFunc create_innov,
        int tries)
{
  ...
  InnovNodeGene *in_node = nullptr;
  // Pointers to the nodes
  InnovNodeGene *out_node = nullptr;
  ...
  // Try to find nodes for link.
  {
    ...
    for (int trycount = 0; !found_nodes and (trycount < tries); trycount++)
    {
      ...
      InnovLinkGene *existing_link =
              find_link(in_node->node_id, out_node->node_id, do_recur);
      if (existing_link not_eq nullptr)
      {
  ...
  // Create the gene.
  {
    InnovationId innov_id(in_node->node_id, out_node->node_id, do_recur);
...
void InnovGenome::add_link(
        std::vector< InnovLinkGene > &llist,
        const InnovLinkGene &l)
{
  auto it = std::upper_bound(llist.begin(), llist.end(), l, linklist_cmp);
  llist.insert(it, l);
}

void InnovGenome::add_node(
...
void InnovGenome::mate_multipoint(
        InnovGenome *genome1,
        InnovGenome *genome2,
        InnovGenome *offspring,
        real_t fitness1,
        real_t fitness2)
{
  ...
  std::vector< InnovLinkGene > &links1 = genome1->links;
  std::vector< InnovLinkGene > &links2 = genome2->links;

  // The baby InnovGenome will contain these new Traits, InnovNodeGenes,
  // and InnovLinkGenes
  offspring->reset();
  std::vector< Trait > &newtraits = offspring->traits;
  std::vector< InnovNodeGene > &newnodes = offspring->nodes;
  std::vector< InnovLinkGene > &newlinks = offspring->links;
  // Checks for link duplication
  std::vector< InnovLinkGene >::iterator curgene2;
  ...
  std::vector< InnovLinkGene >::iterator p1gene;
  std::vector< InnovLinkGene >::iterator p2gene;
  // Innovation numbers for links inside parents' InnovGenomes
  real_t p1innov;
  real_t p2innov;
  // For checking if InnovNodeGenes exist already
  std::vector< InnovNodeGene >::iterator curnode;
  ...
  p1gene = links1.begin();
  p2gene = links2.begin();
  while (!((p1gene == links1.end()) and (p2gene == (links2).end())))
  {
    ...
    else
    {
      ...
      if (std::fabs(p1innov - p2innov) < epsilon)
      {
        ...
        if (((p1gene->enable) == false) or ((p2gene->enable) == false))
        {
          if (rng.prob() < 0.75)
          { disable = true; }
        }
    ...
    while ((curgene2 not_eq newlinks.end())
           and (not((curgene2->in_node_id() == protogene.gene()->in_node_id())
                    and (curgene2->out_node_id() == protogene.gene()->out_node_id())
                    and (curgene2->is_recurrent()
                         == protogene.gene()->is_recurrent())))
           and (not((curgene2->in_node_id() == protogene.gene()->out_node_id())
                    and (curgene2->out_node_id() == protogene.gene()->in_node_id())
                    and (!(curgene2->is_recurrent()))
                    and
                    (!(protogene.gene()->is_recurrent())))))
    {
    ...
    if (curgene2 not_eq newlinks.end())
    { skip = true; }
    ...
    if (not skip)
    {
      ...
      if (inode->node_id < onode->node_id)
      {
        ...
        while ((curnode not_eq newnodes.end())
               and (curnode->node_id not_eq inode->node_id))
        {
          ++curnode;
        }
        ...
        curnode = newnodes.begin();
        while ((curnode not_eq newnodes.end())
               and (curnode->node_id not_eq onode->node_id))
        {
          ++curnode;
        }
        if (curnode == newnodes.end())
      ...
      // End InnovNodeGene checking section- InnovNodeGenes are now
      // in new InnovGenome
...
// TODO: use NodeLookup for newnodes instead of linear search!
void InnovGenome::mate_multipoint_avg(
        InnovGenome *genome1,
        InnovGenome *genome2,
        InnovGenome *offspring,
        real_t fitness1,
        real_t fitness2)
{
  ...
  // This InnovLinkGene is used to hold the average of the two links to
  // be averaged
  InnovLinkGene avgene(0,
                       0,
                       0,
                       0,
                       0,
  ...
  while (!((p1gene == links1.end()) and (p2gene == (links2).end())))
  {
    ...
    else
    {
      ...
      if (std::fabs(p1innov - p2innov) < epsilon)
      {
        ...
        if (rng.prob() > 0.5)
        { avgene.set_recurrent(p1gene->is_recurrent()); }
        else
        { avgene.set_recurrent(p2gene->is_recurrent()); }

        avgene.innovation_num = p1gene->innovation_num;
        avgene.mutation_num =
                (p1gene->mutation_num + p2gene->mutation_num) / 2.0;
    ...
    while ((curgene2 not_eq newlinks.end()))
    {
      if (((curgene2->in_node_id() == protogene.gene()->in_node_id()) and
           (curgene2->out_node_id() == protogene.gene()->out_node_id()) and
           (curgene2->is_recurrent() == protogene.gene()->is_recurrent())) or
          ((curgene2->out_node_id() == protogene.gene()->in_node_id()) and
           (curgene2->in_node_id() == protogene.gene()->out_node_id()) and
           (!(curgene2->is_recurrent())) and
           (!(protogene.gene()->is_recurrent()))))
      {
        skip = true;
      }
...
{
  std::vector< InnovLinkGene > &links1 = this->links;
  std::vector< InnovLinkGene > &links2 = g->links;
...
real_t InnovGenome::trait_compare(
        Trait *t1,
        Trait *t2)
{
  int id1 = t1->trait_id;
...
InnovLinkGene *InnovGenome::find_link(
        int in_node_id,
        int out_node_id,
        bool is_recurrent)
{
  for (InnovLinkGene &g: links)
  {
    if ((g.in_node_id() == in_node_id)
        and (g.out_node_id() == out_node_id)
        and (g.is_recurrent() == is_recurrent))
    {
      return &g;
    }
  }
...
void InnovGenome::delete_if_orphaned_hidden_node(int node_id)
{
  ...
  if ((node == nullptr) or (node->type not_eq NT_HIDDEN))
  {
    return;
  }
  ...
  for (InnovLinkGene &link: links)
  {
    if (link.in_node_id() == node_id or link.out_node_id() == node_id)
    {
      found_link = true;
      break;
    }
...
void InnovNodeGene::set_creator_name(const std::string &creator_name)
{
