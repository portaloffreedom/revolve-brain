...
bool MultiNNSpecies::add_Organism(MultiNNSpeciesOrganism *o)
{
  organisms.push_back(o);
  return true;
}

void MultiNNSpecies::remove_eliminated()
{
  erase_if(organisms, [](MultiNNSpeciesOrganism *org)
  {
    return org->eliminate;
  });
}

void MultiNNSpecies::remove_generation(int gen)
{
  erase_if(organisms, [gen](MultiNNSpeciesOrganism *org)
  {
    return org->generation == gen;
  });
}

MultiNNSpeciesOrganism *MultiNNSpecies::first()
{
...
//   outFile << std::endl << "/* MultiNNSpecies #" << id
//           << " : (Size " << organisms.size()
//           << ") (AF " << ave_fitness
...
//     outFile << std::endl << "/* Organism #" << org->genome->genome_id
//             << " Fitness: " << org->eval.fitness
//             << " Error: " << org->eval.error << " */" << std::endl;
...
void MultiNNSpecies::adjust_fitness()
{
  ...
  for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg)
  {
    ...
    if (age <= 10)
    {
      ((*curorg)->adjusted_fitness) =
              ((*curorg)->adjusted_fitness) * env->age_significance;
    }

    // Do not allow negative fitness
    if (((*curorg)->adjusted_fitness) < 0.0)
    {
      (*curorg)->adjusted_fitness = 0.0001;
    }

    // Share fitness with the species
    (*curorg)->adjusted_fitness =
            ((*curorg)->adjusted_fitness) / (organisms.size());
  }
        ...
        vector<MultiNNSpecies *> &sorted_species)
{
  ...
  // If we have a super_champ (MultiNNSpeciesPopulation champion), finish off
  // some special clones
