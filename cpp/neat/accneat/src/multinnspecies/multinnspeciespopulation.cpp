...
MultiNNSpeciesPopulation::MultiNNSpeciesPopulation(rng_t rng,
...
size_t
MultiNNSpeciesPopulation::size()
{
  return norgs;
}

Organism *
MultiNNSpeciesPopulation::get(size_t index)
{
  return &orgs.curr()[index];
}

unique_ptr<Organism>
MultiNNSpeciesPopulation::make_copy(size_t index)
{
  MultiNNSpeciesOrganism *copy = new MultiNNSpeciesOrganism((MultiNNSpeciesOrganism &)*get(index));
  return unique_ptr<Organism>(copy);
}

void
MultiNNSpeciesPopulation::verify()
{
...
void
MultiNNSpeciesPopulation::spawn()
{
...
void
MultiNNSpeciesPopulation::speciate()
{
  last_species = 0;
...
void
MultiNNSpeciesPopulation::next_generation()
{
#ifndef NDEBUG
  for (MultiNNSpeciesOrganism &org: orgs.curr()) {
  ...
  std::vector<MultiNNSpecies *> sorted_species;  //Species sorted by max fit org in Species
  int half_pop;
