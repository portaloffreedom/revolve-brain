...
#undef __if
                            ...
                            size_t &len_)
  {
    ...
    for (size_t i = 0; i < config.ntrials; i++)
    {
      ...
      create_distance_map(config_,
                          trial.food_pos,
                          trial.dist_map,
                          trial.max_dist);
      uchar dist = trial.dist_map[config.agent_pos.row
                                  * config.width + config.agent_pos.col];
      trial.max_steps = dist + 3 * trial.seqlen;
    }
    memcpy(config_->trials,
           trials.data(),
           sizeof(Config::Trial) * config.ntrials);
  }
