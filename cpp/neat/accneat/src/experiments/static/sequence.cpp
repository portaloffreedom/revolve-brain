...
static struct SequenceInit
{
    SequenceInit()
    {
      create_static_experiment("seq-1bit-2el",
                               ...
                               {
                                   std::string syms = "ab";
                                   std::vector<std::string> seqs = permute_repeat(syms,
                                                                                  2);
                                   return create_parallel_output_tests(syms,
                                                                       seqs);
                               });
  ...
  for (std::string &sequence: sequences) {
    ...
    for (char sym: sequence) {
      //Create step in which symbol is presented
      {
        ...
        append(input,
               X); // Symbol being provided in this step
        append(input,
               _); // Not querying
        append(input,
               sym_encoding[sym]);
        ...
        append(output,
               _,
               sequence_len * nbits); // Empty output

        steps.emplace_back(input,
                           output,
                           weight_seq);
      }
