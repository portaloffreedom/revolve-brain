...
ActivationPartition *
partitions(const RawBuffers &bufs,
           const Offsets &offs)
{
...
public:
    ...
    void
    configure_batch(const RawBuffers &bufs_,
                    const Offsets &offsets_)
    {
      bufs = bufs_;
      offsets = offsets_;

      memcpy(NEAT::links(bufs,
                         offsets),
             gpu_links.data(),
             sizeof(CudaLink) * gpu_links.size());
      memcpy(NEAT::partitions(bufs,
                              offsets),
             partitions.data(),
