...
AsyncNeat::AsyncNeat(unsigned int n_inputs,
        ...
        : n_inputs(n_inputs), n_outputs(n_outputs), generation(1)
          , best_fitness_counter(0), rng_seed(rng_seed), fittest(nullptr)
          , fittest_fitness(std::numeric_limits<float>().min()), robot_name(
                robot_name)
{
...
void AsyncNeat::singleEvalutionFinished(
