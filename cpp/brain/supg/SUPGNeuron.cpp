...
unsigned int
SUPGNeuron::GetDimensionInput(unsigned int n_inputs,
...
unsigned int
SUPGNeuron::GetDimensionOutput(unsigned int n_outputs)
{
  return 2 + n_outputs;
}
...
SUPGNeuron::SUPGNeuron(NEAT::CpuNetwork *cppn,
        ...
        , supg_internal_inputs(GetDimensionInput(0,
                                                 coordinates.size()))
        ...
        , timer_window(cicle_length)
{
  ...
  if (n_input < supg_internal_inputs) {
    std::stringstream ss;
    ss << "SUPGNeuron::SUPGNeuron() Invalid network input size or coordinate vector size (cppn.input_size == "
...
void
SUPGNeuron::reset(float global_time)
{
...
void
SUPGNeuron::init_timer(float global_time)
{
  cppn->load_sensor(Input::TIMER,
                    0);
  load_coordinates();
...
void
SUPGNeuron::set_coordinates(std::vector<float> coordinates)
{
  this->coordinates = coordinates;
}

void
SUPGNeuron::load_coordinates()
{
...
NEAT::real_t
SUPGNeuron::get_timer(float global_time)
{
//     if (coordinates[0] == 0)
...
void
SUPGNeuron::load_sensor(size_t isensor,
                        NEAT::real_t activation)
{
  cppn->load_sensor(isensor + supg_internal_inputs,
                    activation);
}

void
SUPGNeuron::activate(float global_time)
{
