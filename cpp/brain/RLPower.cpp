...
void RLPower::loadPolicy(std::string const policy_path)
{
  ...
  std::cout
          << "velocity: "
          << policy_file[0]["population"][0]["velocity"]
          << std::endl;
...
void RLPower::updatePolicy()
{
  ...
  if (algorithm_type_ == "C" || algorithm_type_ == "D")
  {
    ...
    noise_sigma_ =
            noise_sigma_
            * std::exp(sigma_tau_correction_ * sigma_dist(sigma_mt));
  }
  else
  {
    ...
    if (algorithm_type_ == "B" || algorithm_type_ == "D")
    {
      ...
      for (unsigned int i = 0; i < n_actuators_; i++)
      {
        ...
        for (unsigned int j = 0; j < source_y_size_; j++)
        {
          ...
          spline_point +=
                  ((policy1->at(i)[j] - (*current_policy_)[i][j]))
                  * (fitness1 / total_fitness);
          spline_point +=
                  ((policy2->at(i)[j] - (*current_policy_)[i][j]))
      ...
      for (unsigned int i = 0; i < n_actuators_; i++)
      {
        ...
        for (unsigned int j = 0; j < source_y_size_; j++)
        {
          ...
          for (auto const &it : ranked_policies_)
          {
            ...
            spline_point +=
                    ((policy->at(i)[j] - (*current_policy_)[i][j]))
...
const unsigned int RLPower::MAX_EVALUATIONS = 1000;

/// \brief max length of policies vector
const unsigned int RLPower::MAX_RANKED_POLICIES = 10;

/// \brief number of data points for the interpolation cache
const unsigned int RLPower::INTERPOLATION_CACHE_SIZE = 100;

/// \brief number of initially sampled spline points
const unsigned int RLPower::INITIAL_SPLINE_SIZE = 3;

/// \brief after # generations, it increases the number of spline points
const unsigned int RLPower::UPDATE_STEP = 100;
