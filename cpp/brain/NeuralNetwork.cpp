...
NeuralNetwork::NeuralNetwork() :
        flipState_(false), nInputs_(0), nOutputs_(0), nHidden_(0), nNonInputs_(0)
{
  // Initialize weights, input and states to zero by default
  memset(inputWeights_,
         0,
         sizeof(inputWeights_));
  memset(outputWeights_,
         0,
         sizeof(outputWeights_));
  memset(hiddenWeights_,
         0,
         sizeof(hiddenWeights_));
  memset(state1_,
         0,
         sizeof(state1_));
  memset(state2_,
...
void
NeuralNetwork::step(double time)
{
  unsigned int i = 0;
  unsigned int j = 0;
...
void
NeuralNetwork::update(const std::vector<ActuatorPtr> &actuators,
